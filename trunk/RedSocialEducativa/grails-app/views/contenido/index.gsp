		...
		<div id="list-contenido" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<th><g:message code="contenido.tema.label" default="Tema" /></th>
					
						<g:sortableColumn property="titulo" title="${message(code: 'contenido.titulo.label', default: 'Titulo')}" />
				...
				<g:each in="${contenidoInstanceList}" status="i" var="contenidoInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${contenidoInstance.id}">${fieldValue(bean: contenidoInstance, field: "tema")}</g:link></td>
					
						<td>${fieldValue(bean: contenidoInstance, field: "titulo")}</td>
