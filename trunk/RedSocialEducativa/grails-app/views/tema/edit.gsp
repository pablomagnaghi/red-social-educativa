							...
							<ul class="one-to-many">
								<g:each in="${temaInstance?.contenidos?}" var="c">
	    							<li><g:link controller="contenido" action="show" id="${c.id}"
	    								params="['cursoId': cursoId, 'temaId': temaInstance.id]">${c?.encodeAsHTML()}</g:link></li>
								</g:each>
								<li class="add">
									<g:link controller="contenido" action="create" params="['cursoId': cursoId, 'temaId': temaInstance.id]">
							...
							<ul class="one-to-many">
								<g:each in="${temaInstance?.materiales?}" var="m">
								    <li><g:link controller="materialTema" action="show" id="${m.id}"
								    	params="['cursoId': cursoId, 'temaId': temaInstance.id]">${m?.encodeAsHTML()}</g:link></li>
								</g:each>
								<li class="add">
									<g:link controller="materialTema" action="create" params="['cursoId': cursoId, 'temaId': temaInstance?.id]">
