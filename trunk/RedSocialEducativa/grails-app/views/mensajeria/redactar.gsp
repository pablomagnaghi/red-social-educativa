...
<body>
  ...
  <div id="organigrama" style="display:none">
  	...
  	<div>Cursos Aprendiz</div>
  	<g:each in='${cursosAprendiz}' var='cursoAprendiz'>
  		<div id="divAprendiz-${cursoAprendiz.id}">
  		<img src="../images/tree_plus.gif" class="link-aprendiz" id="${cursoAprendiz.id}" style="cursor: pointer"/>
  		<span style="margin-left: 10px;">${cursoAprendiz.nombre} - ${cursoAprendiz.materia.codigo}</span>
  		<div id="aprendiz-${cursoAprendiz.id}"></div>
  		</div>
  	</g:each>
  </div>
  <div id="spinner" class="center_div" style="display:none; height: 16px;
	width: 16px;">
